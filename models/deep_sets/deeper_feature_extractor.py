...
class DeeperFeatureExtractor(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv4 = nn.Sequential(
            nn.Conv2d(in_channels=out_channels * 4, out_channels=out_channels * 8, kernel_size=3, padding=1),
            nn.BatchNorm2d(out_channels * 8),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels=out_channels * 8, out_channels=out_channels * 8, kernel_size=3, padding=1),
            nn.BatchNorm2d(out_channels * 8),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2)
        )
