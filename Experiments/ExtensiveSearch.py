...
class ESearcher(Assesment):
    ...
    def search_parallel(self, all_bands: List[int],parallel_runs:int,*args,**kwargs) -> Dict[int, float]:
        ...
        for i in rem_idx[:-2]:
            scores = self.assess_parallel(all_bands[i:i+parallel_runs],all_bands,*args,**kwargs)
            for k in range(parallel_runs):
                missing_band_to_score[all_bands[i+k]] = scores[k]
        scores = self.assess_parallel(all_bands[rem_idx[-1]:reminder+rem_idx[-1]],all_bands,*args,**kwargs)
        for k in range(reminder):
            missing_band_to_score[all_bands[rem_idx[-1] + k]] = scores[k]
        return missing_band_to_score

    def assess_once(self,bands:List[int],result_queue:multiprocessing.Queue,*args,**kwargs) -> float:
        print("here:",bands)
        print("here2:",result_queue)
    ...
    def assess_parallel(self, bands_to_test,all_bands,*args,**kwargs) -> List[float]:
        ...
        for band in bands_to_test:
            ...
            p = multiprocessing.Process(target=self.assess_once, kwargs={"bands":almost_all_bands,"result_queue":result_queue})
            p.start()
        ...
        '''for i in range(10):
            # create a process to run function2
            p = multiprocessing.Process(target=self.assess_once, args=(i,))
            p.start()
            # save the process object in a list
            processes.append(p)
        for partial_bands in almost_all_list:
            print(partial_bands)
            # create a process to run function2
            p = multiprocessing.Process(target=self.assess_once, args=par,))
            p.start()
            # save the process object in a list
        ...
        #with Pool() as pool:
        #    res = pool.map(assess_once, almost_all_list)
        #    print(res)
        #    return res
    ...
    def search_all(self,all_bands: List[int],min_bands,parallel_runs=2,*args,**kwargs):
        ...
        while len(bands) > min_bands:
            ...
            logging.info(f"Words band was {len(bands)}, testing bands list of size {str(worst_band)}. score without him={str(score)}")
            best_score[len(bands)] = score
            average_score[len(bands)] = statistics.mean(results.values())
            logging.info(f"Average score for {len(bands)} = {average_score[len(bands)]}")
