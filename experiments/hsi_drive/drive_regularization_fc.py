...
def experiment(model):
    ...
    for index, item in zip(range(limit), iter):
        ...
        get_device()
        train_model(model, train_loader, epochs=1, lr=0.000001, device=get_device(),regularization=True,squeeze=True)
...
if __name__ == '__main__':
    model = RegMlpModel(N_BANDS, NUM_CLASSES_DRIVE, 1e-4)
    experiment(model)
